...
hiddenimports = [
    ...
    'miniaudio',  # 阿里云 TTS 流式 MP3 解码（core/voice/mp3_decoder.py 函数内动态 import，PyInstaller 静态分析漏抓）
...
# 项目本地 ui/core 子模块大量以「函数内 import」形式存在，PyInstaller 静态分析会漏抓
# （运行时 ModuleNotFoundError: No module named 'ui.settings_dialog'）。
# 全量收集本地包，避免逐个补 hiddenimports。
hiddenimports += ['ui']
...
a = Analysis(
    ...
    datas=[
        ...
        # DeepSeek Harness node 闭包 + 默认 cordis.yml + 元数据（frozen 下由 core/llm/harness_bridge.py 的 _runtime_data_dir 定位）
