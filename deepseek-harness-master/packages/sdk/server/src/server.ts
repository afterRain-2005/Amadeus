...
/** Every closed approval outcome, for normalizing the host's wire answer. */
...
export class HarnessSdkJsonRpcServer {
  ...
  constructor(
    ...
    // Approval answerer: forward the pending permission question to the host
    // over JSON-RPC and return its closed outcome. A missing/throwing host
    // answer fails closed (unavailable), and an aborted signal is cancelled.
    this.disposers.push(ctx.on('approval/request', async (req) => {
      if (req.signal?.aborted) return 'cancelled'
      try {
        const outcome = await this.transport.request('approval/request', {
          sessionId: String(req.agent.session.id),
          toolName: req.toolName,
          ...(req.callId === undefined ? {} : { callId: String(req.callId) }),
          ...(req.reason === undefined ? {} : { reason: req.reason }),
        })
        return APPROVAL_OUTCOMES.includes(outcome as ApprovalOutcome)
  ...
  /**
   * Interrupt the active turn on one SDK-owned session. Cancellation aborts the
   * current model request / tool step; the loop converges to idle and the host
   * observes `session.status` → idle, mirroring a user interrupt.
   * @param params - target session.
   * @returns empty JSON-RPC result.
   */
  async cancel(params: { sessionId: string }): Promise<Record<string, never>> {
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      case 'session/cancel':
        return this.cancel(params as unknown as { sessionId: string })
      case 'shutdown':
