...
class DeepSeekHarness:
    ...
    def cancel(self, session_id: str) -> None:
        """Interrupt the active turn on one session from any thread.

        The client transport is write-locked and thread-safe, so a UI thread may
        call this while :meth:`run` is blocked draining notifications; the
        runtime aborts the turn and the run loop observes idle.
        """
        self.start()
        self._client.request("session/cancel", {"sessionId": session_id}, response_model=_CancelResponse)
...
class Session:
    ...
    def run(
        ...
        *,
        ...
        with self.harness.client.subscribe_session_notifications(self.id) as subscription:
            ...
            received = False
            while True:
                # The runtime may send host requests (approval) while the turn
                # runs; drain and answer them so the server-side answerer never
                # hangs waiting for a decision.
                self._drain_host_requests(on_approval)
                try:
                    notification = subscription.next(timeout=0.1)
                except queue.Empty:
                    continue
                if not received:
    ...
    def _drain_host_requests(self, on_approval: Callable[[dict], str] | None) -> None:
        """Answer pending host-originated JSON-RPC requests without blocking."""
        client = self.harness.client
        while True:
            try:
                request = client.next_request(timeout=0)
            except queue.Empty:
                return
            except BaseException:
                return
            if request.method == "approval/request":
                client.respond(request.id, self._resolve_approval(request, on_approval))
            else:
    ...
    @staticmethod
    def _resolve_approval(request, on_approval: Callable[[dict], str] | None) -> str:
        if on_approval is None:
            return "unavailable"
        payload = request.payload
        decision = on_approval({
            "sessionId": payload.get("sessionId"),
            "toolName": payload.get("toolName"),
            "callId": payload.get("callId"),
            "reason": payload.get("reason"),
        })
        if decision in ("allowed-once", "rejected", "cancelled", "unavailable"):
            return decision
        return "unavailable"
