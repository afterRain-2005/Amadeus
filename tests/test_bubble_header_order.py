...
v4：签名扩展为 (header, footer, corners, status_line, x, w, h)：
...
def test_sync_skipped_when_accessories_missing(qapp):
    ...
    desktop_pet._sync_bubble_accessories(None, None, None, None, 10, 100, 40)
...
def test_sync_updates_header_footer_geometry(qapp):
    ...
    desktop_pet._sync_bubble_accessories(header, footer, None, None, 10, 100, 40)
    assert header.geometry() == QRect(10, 4, 100, 16)
    assert footer.geometry() == QRect(10, 30, 100, 16)


def test_sync_caps_header_width(qapp):
    """名牌宽度上限 150，超出时居中收窄（不整行铺满气泡）。"""
    header = QLabel("K U R I S U")
    desktop_pet._sync_bubble_accessories(header, None, None, None, 0, 300, 80)
    assert header.geometry() == QRect(75, 4, 150, 16)


def test_sync_corners_track_bubble_edges(qapp):
    """四角括号应贴在气泡四角（外扩 5px，随几何变化）。"""
    corners = _corners(qapp)
    desktop_pet._sync_bubble_accessories(None, None, corners, None, 10, 200, 60)
    assert corners[0].geometry() == QRect(5, 4, 12, 12)      # ⌈ 左上
    assert corners[1].geometry() == QRect(203, 4, 12, 12)    # ⌉ 右上
    assert corners[2].geometry() == QRect(5, 59, 12, 12)     # ⌊ 左下
    assert corners[3].geometry() == QRect(203, 59, 12, 12)   # ⌋ 右下


def test_sync_status_line_below_bubble(qapp):
    """状态行应位于气泡下方（台词与状态分离）。"""
    status = QLabel("")
    desktop_pet._sync_bubble_accessories(None, None, None, status, 30, 200, 60)
    assert status.geometry() == QRect(38, 72, 184, 14)


def test_sync_does_not_touch_missing_side(qapp):
    """仅一侧存在（早期半构建状态）同样安全：不报错，已存在侧正常同步。"""
    footer = QLabel("wire ESTABLISHED")
    desktop_pet._sync_bubble_accessories(None, footer, None, None, 10, 100, 40)
    assert footer.geometry() == QRect(10, 30, 100, 16)